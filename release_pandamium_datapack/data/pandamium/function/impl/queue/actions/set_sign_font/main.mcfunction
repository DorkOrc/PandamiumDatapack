...
$execute as @a[scores={id=$(source)},limit=1] run function pandamium:impl/font/apply_style/main
...
$execute if score <lines_changed> variable matches 1.. as @a[scores={id=$(source)},limit=1] at @s run playsound ui.cartography_table.take_result master @s
