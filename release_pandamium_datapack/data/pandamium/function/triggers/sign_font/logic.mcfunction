execute unless score @s gameplay_perms matches 6.. run return run function pandamium:misc/print_donator_only_message
...
execute if score @s sign_font matches 1.. run return run tellraw @s [{"text":"[Sign Font]","color":"dark_red"},{"text":" That is not a valid option! Menu input error.","color":"red"}]
...
#
tellraw @s [{"text":"[Sign Font]","color":"dark_red"},{"text":" That is not a valid option! Initial input error.","color":"red"}]
