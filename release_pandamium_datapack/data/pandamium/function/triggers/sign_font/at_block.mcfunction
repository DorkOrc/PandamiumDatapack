...
execute unless score <line> variable matches 1..5 run return run tellraw @s [{"text":"[Sign Font]","color":"dark_red"},{"text":" That is not a valid option! Incorrect line input.","color":"red"}]
...
execute if score <valid_option> variable matches 0 run return run tellraw @s [{"text":"[Sign Font]","color":"dark_red"},{"text":" That is not a valid option! Text editing error.","color":"red"}]
