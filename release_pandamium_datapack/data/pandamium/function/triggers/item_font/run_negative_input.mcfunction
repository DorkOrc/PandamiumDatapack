...
execute if score <valid_option> variable matches 0 run return run tellraw @s [{"text":"[Item Font]","color":"dark_red"},{"text":" That is not a valid option! Text editing error.","color":"red"}]
...
# success
summon item_display ~ ~ ~ {Tags:["item_font"]}
data modify entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] item set from entity @s SelectedItem
execute if data storage pandamium:temp item.components."minecraft:custom_name" run data modify entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] item.components.minecraft:custom_name set from storage pandamium:temp item.components."minecraft:custom_name"
execute unless data storage pandamium:temp item.components."minecraft:custom_name" run data remove entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] item.components.minecraft:custom_name
execute if data storage pandamium:temp item.components."minecraft:lore" run data modify entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] item.components.minecraft:lore set from storage pandamium:temp item.components."minecraft:lore"
execute unless data storage pandamium:temp item.components."minecraft:lore" run data modify entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] item.components.minecraft:lore set value []
item replace entity @s weapon.mainhand from entity @e[limit=1,distance=..0.01,type=item_display,tag=item_font] contents
kill @e[type=item_display,tag=item_font]
