...
$execute if score @s hide_coordinates matches 1 run tellraw @s [{color:"green",text:""},{color:"dark_green",text:"[Homes]"}," Set ",{storage:"pandamium:temp",nbt:"home_name",interpret:true}," at ",{color:"aqua",storage:"pandamium.db.players:io",nbt:"selected.entry.data.homes.$(home).xyzd[0]",plain:true,obfuscated:true,extra:[" ",{storage:"pandamium.db.players:io",nbt:"selected.entry.data.homes.$(home).xyzd[1]",plain:true}," ",{storage:"pandamium.db.players:io",nbt:"selected.entry.data.homes.$(home).xyzd[2]",plain:true}]}," in ",{color:"aqua",nbt:"dimension_name",storage:"pandamium:temp",interpret:true},"!"]
