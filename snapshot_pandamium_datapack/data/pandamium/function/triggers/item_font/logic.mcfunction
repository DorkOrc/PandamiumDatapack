execute unless predicate pandamium:player/has_supporter_perks run tellraw @s [{text:"[Item Font]",color:"dark_red"},{storage:"pandamium:dictionary",nbt:"triggers.supporter_only_trigger",interpret:true}]
execute unless predicate pandamium:player/has_supporter_perks run return run tellraw @s {text:" This command would let you change the colours and fonts of your items!",color:"gray"}
