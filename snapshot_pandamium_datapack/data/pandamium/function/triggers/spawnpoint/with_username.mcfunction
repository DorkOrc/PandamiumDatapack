...
$execute unless score $(username) spawnpoint_x = $(username) spawnpoint_x run return run tellraw @s [{text:"[Spawnpoint]",color:"dark_red"},{text:" ",color:"red",extra:[{storage:"pandamium:temp",nbt:"display_name",interpret:true}," has no known last spawnpoint!"]}]
...
execute if score <do_teleport> variable matches 1 run return run tellraw @s [{text:"[Spawnpoint]",color:"dark_green"},{text:" Teleported to ",color:"green",extra:[{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"display_name",interpret:true}]},"'s last spawnpoint (in spectator mode)!"]}]
