...
execute if score @s staff_perms matches 1.. unless score @s staff_perms matches 2.. unless predicate pandamium:in_spawn unless dimension pandamium:staff_world run return run tellraw @s [{text:"[Toggle Spectator]",color:"dark_red",hover_event:{action:"show_text",text:[{text:"Click to teleport to ",color:"yellow"},{text:"Spawn",bold:true},{text:" in spectator mode"}]},click_event:{action:"run_command",command:"trigger spawn set -1"}},{text:" Helpers may only use spectator mode at spawn and in the staff world!",color:"red"}]
