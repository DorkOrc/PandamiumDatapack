...
execute if score <player_info.returned> variable matches 0 if score @s player_info matches -1 run return run tellraw @s [{"text":"[Player Info]","color":"dark_red"},{"text":" Could not find a non-staff player nearby!","color":"red"}]
...
execute if score <player_info.returned> variable matches 0 if score @s player_info matches 1.. if score <player_info.target_exists> variable matches 0 run return run scoreboard players operation @s player_info_v2 = @s player_info
