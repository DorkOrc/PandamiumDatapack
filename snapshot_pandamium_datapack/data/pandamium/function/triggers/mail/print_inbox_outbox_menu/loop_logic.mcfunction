...
execute if predicate pandamium:mail_list_type/any_outbox_or_drafts if data storage pandamium:temp entry_info{public:1b} run data modify storage pandamium:temp entry_info.receivers set value [{display_name:'"News Feed"'}]
execute if predicate pandamium:mail_list_type/any_outbox_or_drafts unless data storage pandamium:temp entry_info{public:1b} run function pandamium:triggers/mail/print_inbox_outbox_menu/get_player_display_names/main
