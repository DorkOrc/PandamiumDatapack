...
$execute if score <has_access> variable matches 1 unless data storage pandamium.db.mail:io selected.entry.receivers[{id:$(id)}] run return run execute unless score <receiver_type_is_news_feed> variable matches 1 run data modify storage pandamium.db.mail:io selected.entry.indirect_receivers[{id:$(id)}].read set value 1b
