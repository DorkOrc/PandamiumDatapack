...
execute if score <blocked> variable matches 1 run tellraw @a[tag=source,limit=1] [{text:" ",color:"aqua"},{text:"[→]",color:"gray",hover_event:{action:"show_text",value:[{text:"You cannot send a teleport\nrequest to this player.",color:"gray"}]}},{text:" "},{selector:"@s"},{text:" --- "},{score:{name:"@s",objective:"id"},color:"green",bold:true}]
