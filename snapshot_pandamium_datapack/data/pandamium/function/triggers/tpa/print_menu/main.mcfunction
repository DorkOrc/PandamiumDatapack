...
execute if score @s tpa_request.sender_id matches 1.. as @a if score @s id = <sender_id> variable run tellraw @a[tag=source,limit=1] [{text:"\nIncoming Request from ",color:"aqua"},{selector:"@s"},{text:": "},{text:"[✔]",color:"dark_green",hover_event:{action:"show_text",value:[{text:"Click to ",color:"dark_green"},{text:"Accept",bold:true},{text:" incoming TPA Request"}]},click_event:{action:"run_command",command:"trigger tpa set -1"}},{text:" "},{text:"[❌]",color:"dark_red",hover_event:{action:"show_text",value:[{text:"Click to ",color:"dark_red"},{text:"Deny",bold:true},{text:" incoming TPA Request"}]},click_event:{action:"run_command",command:"trigger tpa set -2"}}]
scoreboard players operation <sender_id> variable = @s id
execute as @a if score @s tpa_request.sender_id = <sender_id> variable run tellraw @a[tag=source,limit=1] [{text:"\nOutgoing Request to ",color:"aqua"},{selector:"@s"},{text:": "},{text:"[❌]",color:"dark_red",hover_event:{action:"show_text",value:[{text:"Click to ",color:"dark_red"},{text:"Cancel",bold:true},{text:" outgoing TPA Request"}]},click_event:{action:"run_command",command:"trigger tpa set -3"}}]
