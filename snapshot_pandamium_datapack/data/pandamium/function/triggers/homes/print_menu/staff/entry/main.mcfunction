...
$execute if data storage pandamium.db.players:io selected.entry.data.homes.$(home).name run data modify storage pandamium:temp home_name set value '["",[{"text":"","color":"white","italic":true},{"storage":"pandamium.db.players:io","nbt":"selected.entry.data.homes.$(home).name","interpret":true,"hoverEvent":{"action":"show_text","value":[{"text":"Click to teleport to\\n","color":"blue"},{"storage":"pandamium:temp","nbt":"target","interpret":true},"\'s ",{"text":"Home $(home)","bold":true}]}}]]'
