...
execute unless data storage pandamium.db.click_events:io selected.entry{trigger:"player_info_v2"} run return run tellraw @s [{"text":"[Player Info]","color":"dark_red"},{"text":" That is not a valid option!","color":"red"}]
...
tellraw @s [{"text":"[Player Info]","color":"dark_green"},[{"text":" Teleported to ","color":"green"},[{"score":{"name":"<tp_x>","objective":"variable"},"color":"aqua"}," ",{"score":{"name":"<tp_y>","objective":"variable"}}," ",{"score":{"name":"<tp_z>","objective":"variable"}}]," in ",{"storage":"pandamium:temp","nbt":"dimension_name","color":"aqua"},"!"]]
