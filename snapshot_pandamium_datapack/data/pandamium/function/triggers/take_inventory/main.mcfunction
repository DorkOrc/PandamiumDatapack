...
execute if score <returned> variable matches 0 if score <target_exists> variable matches 0 store success score <returned> variable run tellraw @s [{text:"[Take]",color:"dark_red"},{text:" Could not find a player with ID ",color:"red",extra:[{score:{name:"@s",objective:"take_inventory"}},{text:"!"}]}]

# Run
execute if score <returned> variable matches 0 as @a[tag=target,limit=1] run function pandamium:player/take_items/inventory
execute if score <returned> variable matches 0 if score <item_count> variable matches 1 run tellraw @s [{text:"",color:"yellow"},{text:"[Take]",color:"gold"},{text:" Took "},{text:"1",bold:true,color:"gold"},{text:" item from "},{selector:"@a[tag=target,limit=1]"},{text:"! "},{text:"[→]",color:"blue",hover_event:{action:"show_text",value:[{text:"Click to teleport to ",color:"yellow"},{text:"Staff World",bold:true}]},click_event:{action:"run_command",command:"trigger staff_world set 2"}}]
