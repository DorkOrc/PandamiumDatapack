...
# Lock
execute if score <pose> variable matches -401 store success score <returned> variable run tellraw @a[tag=source,limit=1] [{text:"[Pose]",color:"dark_red"},{text:" Are you sure you want to permanently lock the state of the armour stand you are looking at? ",color:"red"},{text:"[✔]",color:"dark_green",bold:true,hover_event:{action:"show_text",value:[{text:"Click to ",color:"dark_green"},{text:"Lock",bold:true},{text:" the targetted armour stand"}]},click_event:{action:"run_command",command:"trigger pose set -402"}}]
