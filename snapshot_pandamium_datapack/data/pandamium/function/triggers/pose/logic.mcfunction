execute unless score @s gameplay_perms matches 6.. unless score @s staff_perms matches 1.. run return run tellraw @s [{"text":"[Pose]","color":"dark_red"},{"storage":"pandamium:dictionary","nbt":"triggers.supporter_only_trigger","interpret":true}]
execute unless score @s gameplay_perms matches 6.. if score @s staff_perms matches 1.. unless score @s pose matches -403 run return run tellraw @s [{"text":"[Pose]","color":"dark_red"},{"storage":"pandamium:dictionary","nbt":"triggers.supporter_only_trigger","interpret":true,"extra":[" ",{"text":"[🔓]","color":"yellow","clickEvent":{"action":"run_command","value":"/trigger pose set -403"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"yellow"},{"text":"Unlock","bold":true}," the armour stand",{"text":"\n(Staff Only)","color":"#7E7E2A"}]}}]}]
