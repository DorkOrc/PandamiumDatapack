...
execute if score <is_invisible> variable matches 0 if score <can_toggle_invisible> variable matches 1 store success score <returned> variable run tellraw @a[tag=source,limit=1] [{text:"",color:"green"},{text:"[Pose]",color:"dark_green"},{text:" Set target's "},{text:"Invisible",color:"aqua"},{text:" attribute to "},{text:"True",color:"yellow",bold:true},{text:"!"}]
