...
execute if score @s super_secret_trigger_cooldown matches 40.. if data storage pandamium.db.players:io selected.entry.data.custom_effects[{meta:{name:"super_secret_scale"}}] run return run tellraw @s [{text:"[Super Secret]",color:"dark_red"},{text:" Cooldown: ",color:"red",extra:[{score:{name:"<cooldown_seconds>",objective:"variable"},bold:true,extra:[{text:" seconds"}]},{text:"! "}]},{text:"[❌]",color:"dark_red",hover_event:{action:"show_text",text:[{text:"Click to reset your scale",color:"dark_red"}]},click_event:{action:"run_command",command:"trigger super_secret_trigger set -1"}}]
