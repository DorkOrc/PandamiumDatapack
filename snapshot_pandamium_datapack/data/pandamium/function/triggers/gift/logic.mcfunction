...
execute if score <target_exists> variable matches 0 run return run tellraw @s [{text:"[Gift]",color:"dark_red"},{text:" Could not find a player with ID ",color:"red",extra:[{score:{name:"@s",objective:"gift"}},{text:"!"}]}]
...
return run tellraw @s [{text:"",color:"green"},{text:"[Gift]",color:"dark_green"},{text:" Gifted "},{selector:"@a[predicate=pandamium:matches_id,limit=1]"},{text:" a "},{text:"reward credit",color:"aqua"},{text:"! You now have "},{score:{name:"@s",objective:"reward_credits"},bold:true,color:"aqua"},{text:" reward credit"},{text:"(s)",color:"gray"},{text:"! "},{text:"(-",color:"red",extra:[{text:"1",bold:true},{text:")"}]}]
