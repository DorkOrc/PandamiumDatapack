...
execute in pandamium:staff_world run loot replace block 5 0 0 contents loot {pools:[{rolls:1,entries:[{type:"minecraft:item",name:"minecraft:stone"}]}],functions:[{function:"minecraft:set_name",entity:"this",name:{storage:"pandamium.db.click_events:io",nbt:"selected.entry.click_event_root",interpret:true,color:"red",extra:["[Clear]"],hover_event:{action:"show_text",value:{text:"Click to delete all characters from your search",color:"white",extra:[{text:'\n\nNew search would be: "',color:"gray",extra:['"']}]}}}}]}
execute in pandamium:staff_world run data modify storage pandamium:temp keyboard_buttons.clear set from block 5 0 0 item.components.minecraft:custom_name
