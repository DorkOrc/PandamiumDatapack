...
data modify storage pandamium:text input set value {storage:"pandamium.db.click_events:io",nbt:"selected.entry.click_event_root",interpret:true,color:"red",extra:["[←]"],hover_event:{action:"show_text",value:{text:"Click to delete one character from your search",color:"white",extra:[{text:'\n\nNew search would be: "',color:"gray",extra:[{storage:"pandamium.db:click_events",nbt:"selected.entry.data.search"},{text:'"'}]}]}}}
function pandamium:utils/text/input/resolve
data modify storage pandamium:temp keyboard_buttons.delete set from storage pandamium:text input
