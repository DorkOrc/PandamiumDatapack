...
# few results
data modify storage pandamium:temp display_result set value []
...
execute if score <results> variable matches 1 run tellraw @s [{text:""},{text:'1 Result for "',color:"green",extra:[{storage:"pandamium:templates",nbt:"macro.search.search"},{text:'": '}]},{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"display_results[]",interpret:true,separator:{text:", ",color:"gray"}}]}]
execute if score <results> variable matches 2..10 run tellraw @s [{text:""},{score:{name:"<results>",objective:"variable"},color:"green",extra:[{text:' Results for "'},{storage:"pandamium:templates",nbt:"macro.search.search"},{text:'": '}]},{text:"",color:"aqua",extra:[{storage:"pandamium:temp",nbt:"display_results[]",interpret:true,separator:{text:", ",color:"gray"}}]}]
