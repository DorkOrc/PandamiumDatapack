...
execute if predicate pandamium:in_void_dimension anchored eyes positioned ^ ^ ^ as @e[type=trident,predicate=pandamium:periodic_tick/on_load,distance=..10] if items entity @s contents trident[enchantments~[{enchantments:"minecraft:loyalty"}]] unless predicate {condition:"minecraft:entity_properties",entity:"this",predicate:{passenger:{predicates:{"minecraft:custom_data":{custom_entity_type:"trident_loyalty_monitor"}}}}} run function pandamium:detect/use_trident/as_trident
