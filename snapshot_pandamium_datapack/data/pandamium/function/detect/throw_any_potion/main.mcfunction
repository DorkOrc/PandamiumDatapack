...
execute at @s[scores={gameplay_perms=6..,optn.do_projectile_trails=1,active_particles=1..}] anchored eyes positioned ^ ^ ^ as @e[type=#pandamium:potion,predicate=pandamium:periodic_tick/on_load,distance=..10] run function pandamium:impl/database/cache/modify/add_projectile_trail_entry/main
