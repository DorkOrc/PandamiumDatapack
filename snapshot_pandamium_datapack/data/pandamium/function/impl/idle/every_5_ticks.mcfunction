...
scoreboard players remove @a[scores={idle.time=1..}] yearly_playtime_ticks 5

execute as @a[scores={idle.time=1..5}] unless score @s hidden matches 1.. run function pandamium:player/update_tablist_value
