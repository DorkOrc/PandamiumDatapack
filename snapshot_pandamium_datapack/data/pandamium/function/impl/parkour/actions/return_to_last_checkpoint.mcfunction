...
execute if score @s parkour.checkpoint matches 300 in overworld positioned -300.5 136 197.5 rotated 0 30 run function pandamium:utils/teleport/here
tag @s add source
execute unless score @s parkour.checkpoint matches 200 unless score @s parkour.checkpoint matches 300 as @e[type=marker,tag=parkour.node,x=-512,y=-64,z=-512,dx=1024,dy=384,dz=1024] if score @s parkour.node_id = <node_id> variable at @s as @a[tag=source,limit=1] run function pandamium:utils/teleport/here
tag @s remove source
