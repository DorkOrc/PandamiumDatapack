...
# feedback
execute if score <feedback_teleports> global matches 1 run tellraw @a[scores={send_extra_debug_info=2..}] [{text:"[Pandamium: Teleported ",color:"gray",italic:true,hover_event:{action:"show_text",text:{storage:"pandamium:templates",nbt:"teleport"}}},{selector:"@s"},{text:" by ["},{storage:"pandamium:templates",nbt:"teleport.source"},{text:"]]"}]
