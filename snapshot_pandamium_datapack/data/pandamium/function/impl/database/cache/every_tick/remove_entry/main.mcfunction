...
$tellraw @a[scores={tpa_request.sender_id=$(id)}] [{text:"",color:"green"},{text:"[TPA] ",color:"blue"},{storage:"pandamium:temp",nbt:"display_name",interpret:true},{text:" canceled",color:"aqua"},{text:" their TPA request by leaving the game!"}]

$execute store result score <id> variable run scoreboard players get $(username) tpa_request.sender_id
execute if score <id> variable matches 1.. run tellraw @a[predicate=pandamium:matches_id,limit=1] [{text:"",color:"green"},{text:"[TPA] ",color:"blue"},{storage:"pandamium:temp",nbt:"display_name",interpret:true},{text:" denied",color:"aqua"},{text:" your TPA request by leaving the game!"}]
