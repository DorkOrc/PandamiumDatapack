...
$execute unless items entity @s contents *[custom_name] if items entity @s contents *[item_name] run item modify entity @s contents {function:"set_name",entity:"this",name:{entity:"@s",nbt:"Item.components.minecraft:item_name",interpret:true,insertion:"$(stored_item_id)",hoverEvent:{action:"show_item",contents:$(filtered_item)}}}
$execute unless items entity @s contents *[custom_name] if items entity @s contents player_head[profile] if data storage pandamium:local functions."pandamium:utils/database/mail/modify/attach_item".item.components."minecraft:profile".name run item modify entity @s contents {function:"set_name",entity:"this",name:{translate:"block.minecraft.player_head.named",fallback:"%s's Head",with:[{storage:"pandamium:local",nbt:'functions."pandamium:utils/database/mail/modify/attach_item".item.components."minecraft:profile".name'}],insertion:"$(stored_item_id)",hoverEvent:{action:"show_item",contents:$(filtered_item)}}}
