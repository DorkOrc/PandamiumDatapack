...
# else:
tellraw @s [{text:""},{nbt:"slot_prefix",storage:"pandamium:containers",color:"aqua",underlined:true,font:"uniform",extra:[{score:{name:"<display_slot>",objective:"variable"}}]},{text:": "},{score:{name:"<count>",objective:"variable"},color:"gold"},{text:" "},{nbt:"item.display_id",storage:"pandamium:containers",color:"yellow"},{nbt:"plural",storage:"pandamium:containers",color:"gray"}]
