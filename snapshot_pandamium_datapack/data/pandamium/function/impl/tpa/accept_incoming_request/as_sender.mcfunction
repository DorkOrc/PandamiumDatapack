...
execute if score <can_accept> variable matches 0 store success score <returned> variable run tellraw @a[tag=receiver,limit=1] [{text:"[TPA]",color:"dark_red"},{text:" You cannot accept this TPA request currently! ",color:"red"},{text:"[❌]",color:"dark_red",hover_event:{action:"show_text",value:[{text:"Click to ",color:"red"},{text:"Deny",bold:true},{text:" incoming TPA Request"}]},click_event:{action:"run_command",command:"trigger tpa set -2"}}]
