...
$execute if score <selected_entry_sorting_value> variable = <sorting_value> variable run data modify storage pandamium.leader_boards:data leader_boards.$(type).active.entries[$(selected_index)].users append from storage pandamium:local functions."pandamium:impl/leader_boards/update_place/*".entry.users[0]
