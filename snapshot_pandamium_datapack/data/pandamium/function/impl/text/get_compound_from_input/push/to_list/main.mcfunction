...
execute unless data storage pandamium:temp push_source{type:"compound"} unless data storage pandamium:temp push_source{type:"integer"} run function pandamium:impl/text/get_compound_from_input/raise_error {message:{text:"Illegal push: Cannot push ",extra:[{storage:"pandamium:temp",nbt:"push_source.type",bold:true},{text:" into a list"}]}}
