...
execute as @a[scores={just_dropped_dyed_bedrock_breaker=1}] if items entity @s player.cursor #bundles run item modify entity @s player.cursor {function:"minecraft:modify_contents",component:"minecraft:bundle_contents",modifier:{function:"minecraft:reference",name:"pandamium:fix_bedrock_breaker"}}
