...
$execute if score <value> variable matches 0 if data storage pandamium.leader_boards:data leader_boards."$(type)"{source_objective:"parkour_3.best_time_real"} if score $(username) parkour_3.best_time matches 1.. run scoreboard players set <flag_as_unconfirmed> variable 1
...
# remove excess entry
$data remove storage pandamium.leader_boards:data leader_boards.$(type).entries[$(max_entries)]

# remove empty entries
$data modify storage pandamium.leader_boards:data leader_boards.$(type).entries[].not_empty set value 0b
$data modify storage pandamium.leader_boards:data leader_boards.$(type).entries[{players:[{}]}].not_empty set value 1b
$data remove storage pandamium.leader_boards:data leader_boards.$(type).entries[{not_empty:0b}]
$data remove storage pandamium.leader_boards:data leader_boards.$(type).entries[].not_empty
