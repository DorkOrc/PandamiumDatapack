...
execute if score <max_index> variable matches 50.. run return run tellraw @s [{text:"[Font]",color:"dark_red"},{text:" String is too long! As a lag prevention measure, the maximum string length for applying a gradient is 50 characters.",color:"red",hover_event:{action:"show_text",text:[{text:"Failed to apply gradient to ",color:"red"},{storage:"pandamium:temp",nbt:"text",interpret:true}]}}]
