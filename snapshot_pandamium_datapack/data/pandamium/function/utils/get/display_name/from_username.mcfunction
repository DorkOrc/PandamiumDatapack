...
execute if score <player_is_online> variable matches 0 run data modify storage pandamium:text input set value ["",{storage:"pandamium:temp",nbt:"username",extra:[{color:"gray",text:" (Offline)"}],hover_event:{action:"show_text",value:{color:"gray",text:"Shift+Click to insert User ID"}}}]
execute if score <player_is_online> variable matches 1 run data modify storage pandamium:text input set value ["",{selector:"@a[predicate=pandamium:matches_id,limit=1]"}]
data modify storage pandamium:text input[].insertion set string storage pandamium:local functions."pandamium:utils/get/display_name/*".id

function pandamium:utils/text/input/resolve
data modify storage pandamium:temp display_name set from storage pandamium:text input
