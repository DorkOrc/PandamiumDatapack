...
$execute unless score $(username) playtime_ticks matches 0.. run tellraw @a[scores={staff_perms=1..}] [{text:""},{text:"[Staff Info]",color:"dark_gray",hover_event:{action:"show_text",value:"service-name: $(service)"}},{text:" Blocked vote message from $(service) for ",color:"gray",extra:[{text:"$(username)",color:"yellow"},{text:"!"}]}]
