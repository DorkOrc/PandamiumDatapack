...
# welcome messages
title @s title {text:"🐼 ",font:"minecraft:uniform",extra:[{text:"Welcome to Pandamium",bold:true},{text:" 🐼"}]}
title @s subtitle "Please read the Rules"
tellraw @a [{text:"[Pandamium]",color:"blue"},{text:" Welcome to the server, ",color:"dark_aqua",extra:[{selector:"@s"},{text:"! Have fun!"}]}]
tellraw @s [{text:"[Private Info]",color:"blue",hover_event:{action:"show_text",text:[{text:"Click to join our ",color:"aqua"},{text:"Discord Server",bold:true}]},click_event:{action:"open_url",url:"http://discord.pandamium.eu"}},{text:" Use the ",color:"green",extra:[{text:"RTP",color:"aqua"},{text:" at spawn and "},{text:"read the guidebook",color:"aqua"},{text:" to get started. "},{text:"Click here to join our Discord Server",color:"aqua"},{text:" for the full rules and announcements about the server!"}]}]
...
execute unless entity @s[predicate=pandamium:in_spawn] run tellraw @s [{text:"",color:"#ffadb1"},{text:"[Name Change Notice]",color:"#FF424D"},{text:" If you have changed your username since you last joined this server, you may need a staff member to transfer data such as homes, playtime, and reward credits to your new username. If so, please contact staff on our "},{nbt:"hyperlink.discord",storage:"pandamium:dictionary",interpret:true},{text:" server."}]
execute unless entity @s[predicate=pandamium:in_spawn] run tellraw @a[scores={staff_perms=1..}] [{text:"",color:"gray"},{text:"[Staff Info]",color:"dark_gray"},{text:" "},{selector:"@s",color:"gray"},{text:" may have changed their username."}]

# inform staff
tellraw @a[scores={staff_perms=1}] [{text:"",color:"gray"},{text:"[Staff Info] ",color:"dark_gray"},{selector:"@s",color:"gray"},{text:"'s id is "},{score:{objective:"id",name:"@s"},bold:true},{text:"! "},{text:"[→]",color:"blue",hover_event:{action:"show_text",text:[{text:"Click to teleport to ",color:"blue"},{text:"Spawn",bold:true},{text:" in spectator mode"}]},click_event:{action:"run_command",command:"trigger spawn set -1"}}]
