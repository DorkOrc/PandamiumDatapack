...
execute store success score <returned> variable unless score <item_count> variable matches 1.. run tellraw @p[tag=source] [{"text":"[Take] ","color":"dark_red"},{"selector":"@s","color":"red"},{"text":" has no items in their inventory!","color":"red"}]
