...
execute if score <hit> variable matches 0 as @e[type=interaction,dx=0,tag=edit_item_frame.interaction] positioned ~-1 ~-1 ~-1 if entity @s[dx=0] store success score <hit> variable store success score <raycast_hit_target> variable run tag @s add raycast.target
