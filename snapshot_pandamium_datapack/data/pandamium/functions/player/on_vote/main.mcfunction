...
$execute if score $(username) playtime_ticks matches 0.. run tellraw @a[scores={send_extra_debug_info=1..}] [{"text":"[Voting: Relayed vote message from ","color":"gray","italic":true},{"text":"$(username)","color":"red"},"]"]
