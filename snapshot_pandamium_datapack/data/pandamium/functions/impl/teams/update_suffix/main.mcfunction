...
execute unless score @s hide_trophy_suffix matches 1 unless data storage pandamium.db:players selected.entry.data.flair if score @s monthly_votes >= <monthly_votes_leaderboard_highest_value> global run data modify storage pandamium:temp arguments.suffix_crown set value '[" ",{"text":"🜲","color":"gold","hoverEvent":{"action":"show_text","contents":{"text":"This player is at the top of a\\nmonthly leaderboard.","color":"gold"}}}]'
execute unless score @s hide_trophy_suffix matches 1 unless data storage pandamium.db:players selected.entry.data.flair if score @s monthly_playtime_ticks >= <monthly_playtime_leaderboard_highest_value> global run data modify storage pandamium:temp arguments.suffix_crown set value '[" ",{"text":"🜲","color":"gold","hoverEvent":{"action":"show_text","contents":{"text":"This player is at the top of a\\nmonthly leaderboard.","color":"gold"}}}]'
