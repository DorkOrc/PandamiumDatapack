...
execute unless score <item_count> variable matches 1.. store success score <returned> variable run tellraw @a[tag=source,limit=1] [{"text":"[Take] ","color":"dark_red"},{"selector":"@s","color":"red"},{"text":" has no items in their ender chest!","color":"red"}]
