execute unless score @s staff_perms matches 2.. store success score <returned> variable run tellraw @s [{"text":"[Spawnpoint]","color":"dark_red"},{"text":" You do not have access to that functionality!","color":"red"}]
...
execute if score <returned> variable matches 0 as @a[tag=target,limit=1] if score @s spawnpoint_x = @s spawnpoint_x store success score <spawnpoint_exists> variable run function pandamium:impl/triggers/spawnpoint/load_coordinates
