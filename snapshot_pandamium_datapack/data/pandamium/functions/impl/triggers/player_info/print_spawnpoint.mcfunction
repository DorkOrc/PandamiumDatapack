...
execute if score @s spawnpoint_x = @s spawnpoint_x if score <has_spawnpoint> variable matches 0 run tellraw @a[tag=player_info.source,limit=1] [{"text":"Last Spawnpoint: ","color":"gold","hoverEvent":{"action":"show_text","value":[{"text":"Click to teleport to target's ","color":"yellow"},{"text":" Spawnpoint","color":"gold","bold":true}]},"clickEvent":{"action":"run_command","value":"/trigger spawnpoint set -1"}},[{"score":{"name":"@s","objective":"spawnpoint_x"},"bold":true,"color":"yellow"}," ",{"score":{"name":"@s","objective":"spawnpoint_y"}}," ",{"score":{"name":"@s","objective":"spawnpoint_z"}}]," in ",{"nbt":"print_dimension","storage":"pandamium:temp","color":"yellow","bold":true}]
