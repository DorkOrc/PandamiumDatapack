...
execute if data entity @s {Invisible:1b} as @a[tag=source,limit=1] unless predicate pandamium:holding_an_item store success score <cannot_swap> variable store success score <returned> variable run tellraw @s [{"text":"","color":"red"},{"text":"[Pose]","color":"dark_red"}," You cannot take items off of an armour stand while it is invisible!"]
