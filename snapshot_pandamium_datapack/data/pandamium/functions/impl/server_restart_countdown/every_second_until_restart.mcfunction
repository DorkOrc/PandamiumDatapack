...
# join message
tellraw @a[scores={online_ticks=1..20}] [{"text":"[Warning]","color":"dark_red"},[{"text":" The server will be temporarily closed in\n ","color":"red"},{"storage":"pandamium:temp","nbt":"time_left","interpret":true,"bold":true,"color":"yellow"},{"storage":"pandamium:global","nbt":"restart_reason","interpret":true},"!"]]
execute as @a[scores={online_ticks=1..20}] at @s run playsound entity.experience_orb.pickup master @s ~ ~ ~ 1 2 1
