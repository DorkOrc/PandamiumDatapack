...
$data modify storage pandamium:temp xyzd set from storage pandamium:queue this.entries[-1].data.homes.$(home).xyzd
...
execute store result storage pandamium.db:regions selected.chunk.entry.homes[-1].data[0] int 1 run data get storage pandamium:queue this.entries[-1].id
$data modify storage pandamium.db:regions selected.chunk.entry.homes[-1].data[1] set value $(home)

$tellraw @a[scores={send_extra_debug_info=2..}] [{"color":"gray","hoverEvent":{"action":"show_text","contents":[{"storage":"pandamium.db:regions","nbt":"selected.chunk.entry"},"\n\n",[{"color":"gray","text":"at index "},{"storage":"pandamium.db:regions","nbt":"selected.chunk.index"}]]},"italic":true,"text":"[queue.database.datafixer: Appended "},{"storage":"pandamium:queue","nbt":"this.entries[-1].username"},"'s home $(home) to chunk ",{"storage":"pandamium.db:regions","nbt":"selected.chunk.entry.chunk[]","separator":" "},"]"]
