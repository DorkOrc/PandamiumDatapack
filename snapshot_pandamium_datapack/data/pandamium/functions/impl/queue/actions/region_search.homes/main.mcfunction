...
tellraw @a[scores={send_extra_debug_info=2..}] [{"color":"gray","italic":true,"text":"[Pandamium: queue/region_search.homes: Completed region search; Found ","hoverEvent":{"action":"show_text","contents":[{"storage":"pandamium:temp","nbt":"nearby_homes[].message","interpret":true,"separator":"\n"}]}},{"score":{"name":"<count>","objective":"variable"}}," homes within 500 blocks]"]
