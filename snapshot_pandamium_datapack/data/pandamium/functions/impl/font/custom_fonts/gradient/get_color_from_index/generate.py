...
def generate_tree(section,a=None,b=None):
    ...
    if a == b:
        ...
        #return f"execute if score <color_index> variable matches {a} run function pandamium:impl/font/custom_fonts/gradient/tree/{a}\n"
        ...
        with open(('main.mcfunction' if (a == 0 and b == len(colors)-1) else f'tree/{a}..{b}.mcfunction'),'w') as file:
            file.write(generate_tree(section[:L//2],a,a+L//2-1))
            file.write(generate_tree(section[L//2:],a+L//2,b))
        return f"execute if score <color_index> variable matches {a}..{b} run function pandamium:impl/font/custom_fonts/gradient/get_color_from_index/tree/{a}..{b}\n"
