...
execute if score <returned> variable matches 0 as @a if score @s tpa_request.sender_id = <sender_id> variable store success score <returned> variable run tellraw @a[tag=sender,limit=1] [{"text":"[TPA]","color":"dark_red"},[{"text":" You have already sent a TPA request to ","color":"red"},{"selector":"@s","color":"red"},"! ",{"text":"[❌]","bold":true,"color":"gray","clickEvent":{"action":"run_command","value":"/trigger tpa set -3"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"gray"},{"text":"Cancel","bold":true}," outgoing TPA Request"]}}]]
