...
execute unless score <count> variable matches 2.. in pandamium:staff_world run item modify block 5 1 0 contents {function: "minecraft:set_name",name: {storage: "pandamium.db:mail", nbt: "selected.entry.data.items[-1].name", interpret: true, hoverEvent: {action: "show_text", contents: {"text":"Invalid Item Components","color":"gray"}}}}
$execute if score <count> variable matches 2.. in pandamium:staff_world run item modify block 5 1 0 contents {function: "minecraft:set_name",name: [{"text": "$(count) "}, {storage: "pandamium.db:mail", nbt: "selected.entry.data.items[-1].name", interpret: true, hoverEvent: {action: "show_text", contents: {"text":"Invalid Item Components","color":"gray"}}}]}
