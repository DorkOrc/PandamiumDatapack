...
#> Pre
$execute if data storage pandamium.db:players username_indexes."$(username)" run tellraw @s [{"text":"[admin/add_to_database]","color":"dark_red"},{"text":" $(username) is already in [pandamium.db:players]!","color":"red"}]
$execute if data storage pandamium.db:players username_indexes."$(username)" run return 0
