...
$execute store result score @s db.players.index store result storage pandamium.db:players uuid_indexes."$(uuid0)_$(uuid1)_$(uuid2)_$(uuid3)" int 1 store result storage pandamium:templates macro.username__index__id.index int 1 if data storage pandamium.db:players entries[]
...
execute store result storage pandamium.db:players entries[-1].id int 1 store result storage pandamium:templates macro.username__index__id.id int 1 run scoreboard players get @s id
