...
# load if not already
execute unless data storage pandamium.db:players selected run function pandamium:utils/database/players/load/self
...
$data modify storage pandamium:temp arguments.plain_name set from storage pandamium.db:players selected.entry.data.homes.$(home).plain_name
...
$execute if data storage pandamium.db:players selected.entry.data.homes.$(home).name run function pandamium:impl/database/cache/refresh_personal_home_trigger/add_new_trigger with storage pandamium:temp arguments
