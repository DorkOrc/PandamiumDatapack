...
$scoreboard players enable @a[scores={staff_perms=1..,optn.disable_dynamic_triggers.punishments=0}] kick.$(username)

# ban trigger
$scoreboard players set @a[scores={ban.$(username)=1..}] ban $(id)
$scoreboard players set @a[scores={staff_perms=2..}] ban.$(username) 0
$scoreboard players enable @a[scores={staff_perms=2..,optn.disable_dynamic_triggers.punishments=0}] ban.$(username)
...
$execute unless score @s jailed matches 1.. run scoreboard players enable @a[scores={staff_perms=1..,optn.disable_dynamic_triggers.punishments=0}] jail.$(username)
...
$execute if score @s jailed matches 1.. run scoreboard players enable @a[scores={staff_perms=1..,optn.disable_dynamic_triggers.punishments=0}] unjail.$(username)
...
$scoreboard players set @a[scores={staff_perms=1..}] player_info.$(username) 0
$scoreboard players enable @a[scores={staff_perms=1..,optn.disable_dynamic_triggers.player_info=0}] player_info.$(username)
