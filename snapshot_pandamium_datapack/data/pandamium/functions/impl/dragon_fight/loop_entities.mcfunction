execute in the_end as @e[x=0,y=64,z=0,distance=..300,type=enderman,team=!dragon_fight] store result entity @s Attributes[{Name:"minecraft:generic.scale"}].Base double 0.1 store result entity @s Attributes[{Name:"minecraft:generic.max_health"}].Base double 2 store result entity @s Health float 2 run random value 5..100
