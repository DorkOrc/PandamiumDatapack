...
execute if predicate pandamium:environment/is_night if score <month> global matches 12 if score <day> global matches 24.. in overworld as @a[x=0,gamemode=survival,sort=random,limit=1] at @s run function pandamium:misc/summon_random_christmas_mob
