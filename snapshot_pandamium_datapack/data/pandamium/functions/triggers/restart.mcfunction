...
execute if score @s restart matches -2 if score @s staff_alt matches 1.. run tellraw @a[scores={staff_perms=1..}] [{"text":"[Restart] ","color":"red"},"A staff member has scheduled a server restart! ",{"text":"[X]","bold":true,"color":"dark_red","hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"dark_red"},{"text":"Cancel","bold":true}," the restart"]},"clickEvent":{"action":"run_command","value":"/trigger restart set -3"}}]
