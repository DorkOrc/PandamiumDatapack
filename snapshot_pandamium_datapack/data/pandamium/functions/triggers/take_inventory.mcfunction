...
execute if score <returned> variable matches 0 if score @s take_inventory matches 1 store success score <returned> variable run function pandamium:misc/print_nearest_non_staff_player
...
# Run
execute if score <returned> variable matches 0 as @p[tag=target] in pandamium:staff_world run function pandamium:misc/take/take_inventory
execute if score <returned> variable matches 0 run tellraw @s [{"text":"","color":"yellow","clickEvent":{"action":"run_command","value":"/trigger staff_world set 2"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to teleport to ","color":"yellow"},{"text":"Staff World","bold":true}]}},{"text":"[Take]","color":"gold"}," Took ",{"score":{"name":"<item_count>","objective":"variable"},"bold":true,"color":"gold"}," items from ",[{"selector":"@p[tag=target]","clickEvent":{"action":"run_command","value":"/trigger staff_world set 2"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to teleport to ","color":"yellow"},{"text":"Staff World","bold":true}]}},"'s "],{"text":"inventory","bold":true},"!"]
