...
# Run
execute if score <returned> variable matches 0 as @p[tag=target] run function pandamium:utils/take/inventory
execute if score <returned> variable matches 0 if score <item_count> variable matches 1 run tellraw @s [{"text":"","color":"yellow"},{"text":"[Take]","color":"gold"}," Took ",{"text":"1","bold":true,"color":"gold"}," item from ",{"selector":"@p[tag=target]"},"! ",{"text":"[→]","color":"blue","bold":true,"hoverEvent":{"action":"show_text","value":[{"text":"Click to teleport to ","color":"yellow"},{"text":"Staff World","bold":true}]},"clickEvent":{"action":"run_command","value":"/trigger staff_world set 2"}}]
