...
execute if score @s gameplay_perms matches 5.. unless score @s hide_unset_homes matches 1 run tellraw @s \
    [{"text":"\nShow Unset Homes: ","color":"aqua","hoverEvent":{"action":"show_text","value":[{"text":"Click to cycle options for\n","color":"aqua"},{"text":"Show Unset Homes","bold":true},{"text":"\n• On (Default)\n• Off","color":"dark_gray"}]},"clickEvent":{"action":"run_command","value":"/trigger options set -601"}},{"text":"On","color":"yellow","bold":true}]
execute if score @s gameplay_perms matches 5.. if score @s hide_unset_homes matches 1 run tellraw @s ["",\
