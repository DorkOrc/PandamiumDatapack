...
tellraw @a[scores={staff_perms=1..}] [{"text":"[Restart] ","color":"red"},{"nbt":"source","storage":"pandamium:temp","interpret":true}," scheduled a server restart! ",{"text":"[❌]","color":"dark_red","hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"dark_red"},{"text":"Cancel","bold":true}," the restart"]},"clickEvent":{"action":"run_command","value":"/trigger restart_server set -3"}}]
