...
tellraw @a[tag=source,limit=1] [{"text":" ","color":"aqua"},[{"nbt":"session_data.click_events.tpa","storage":"pandamium:temp","interpret":true},{"text":"[→]","color":"blue","bold":true,"hoverEvent":{"action":"show_text","contents":[{"text":"Click to send a teleport request\nto ","color":"blue"},{"selector":"@s","color":"blue","bold":true}]}}]," ",{"selector":"@s"}," --- ",{"score":{"name":"@s","objective":"id"},"color":"green","bold":true}]
