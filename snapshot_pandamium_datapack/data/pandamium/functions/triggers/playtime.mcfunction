...
execute if score <returned> variable matches 0 store success score <returned> variable if score @s playtime matches -1 unless score @s playtime_ticks >= #ticks_per_day constant run tellraw @s [{"text":"[Playtime]","color":"dark_red"},{"text":" That is not a valid option!","color":"red"}]
