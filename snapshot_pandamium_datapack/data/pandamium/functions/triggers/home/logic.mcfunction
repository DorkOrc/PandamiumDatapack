...
execute unless score <home> variable matches 1..25 run return run tellraw @s [{"text":"[Homes]","color":"dark_red"},{"text":" That is not a valid option! ","color":"red"},{"text":"[≡]","color":"blue","bold":true,"hoverEvent":{"action":"show_text","contents":{"text":"Click to see your homes","color":"blue"}},"clickEvent":{"action":"run_command","value":"/trigger homes"}}]
