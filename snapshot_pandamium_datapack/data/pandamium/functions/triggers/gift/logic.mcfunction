...
# select player
scoreboard players operation <id> variable = @s gift
execute unless entity @a[predicate=pandamium:matches_id,limit=1] run return run tellraw @s [{"text":"[Gift]","color":"dark_red"},[{"text":" Could not find a player with ID ","color":"red"},{"score":{"name":"<id>","objective":"variable"}},"!"]]
