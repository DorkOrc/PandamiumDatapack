...
tellraw @a[tag=source,limit=1] [{"text":" ","color":"aqua"},[{"nbt":"session_data.click_events.gift","storage":"pandamium:temp","interpret":true},{"text":"[🎁]","color":"#FF007F","hoverEvent":{"action":"show_text","contents":[{"text":"Click to gift a reward credit to\n","color":"#FF007F"},{"selector":"@s","color":"#FF007F","bold":true}]}}]," ",{"selector":"@s"}," --- ",{"score":{"name":"@s","objective":"id"},"color":"green","bold":true}]
