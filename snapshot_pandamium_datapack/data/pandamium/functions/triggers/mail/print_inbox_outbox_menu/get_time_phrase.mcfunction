...
execute store result score <hour_id> variable run data get storage pandamium.db:mail selected.entry.hour_id
function pandamium:utils/get/date/from_hour_id_score
execute if score <hours_since_sent> variable matches 24.. run return run data modify storage pandamium:temp time_phrase set value '["on ",{"storage":"pandamium:temp","nbt":"date[2]"},"/",{"storage":"pandamium:temp","nbt":"date[1]"},"/",{"storage":"pandamium:temp","nbt":"date[0]"}," at ≈",{"storage":"pandamium:temp","nbt":"date[3]"},":00"]'
