...
data modify storage pandamium.db:mail selected.entry.data.items[].not_taken set value 1b
data modify storage pandamium.db:mail selected.entry.data.items[{taken:1b}].not_taken set value 0b
execute store success score <has_available_attached_items> variable if data storage pandamium.db:mail selected.entry.data.items[{not_taken:1b}]
...
execute if score <has_available_attached_items> variable matches 0 run tellraw @s ["",{"text":"Attachments:\n• ","color":"gray"},{"storage":"pandamium.db:mail","nbt":"selected.entry.data.items[].name","interpret":true,"separator":{"text":"\n• ","color":"gray","strikethrough":false},"strikethrough":true}]
