...
data modify storage pandamium.db:mail selected.entry.data.items[].available set value 1b
execute if data storage pandamium.db:mail selected.entry.data.items[{taken:1b}] run data modify storage pandamium.db:mail selected.entry.data.items[{taken:1b}].available set value 0b
execute store success score <has_available_attached_items> variable if data storage pandamium.db:mail selected.entry.data.items[{available:1b}]
data remove storage pandamium.db:mail selected.entry.data.items[].available
...
execute if data storage pandamium.db:mail selected.entry.data.items[0] if score <has_available_attached_items> variable matches 0 run tellraw @s ["",{"text":"Attachments:\n• ","color":"gray"},{"storage":"pandamium.db:mail","nbt":"selected.entry.data.items[].name","interpret":true,"separator":{"text":"\n• ","color":"gray","strikethrough":false},"strikethrough":true}]
