...
execute unless score @s staff_perms matches 2.. store result score <joined_since_mail_added> variable run function pandamium:triggers/mail/check_if_user_can_receive_mail with storage pandamium.db.players:io selected.entry
execute unless score @s staff_perms matches 2.. if score <joined_since_mail_added> variable matches 0 run return run tellraw @s [{"text":"[Mail]","color":"dark_red"},[{"text":" That player has not joined since mailing was enabled! Once they join the game at least once, they will be elligible for receiving mail.","color":"red"}]]
