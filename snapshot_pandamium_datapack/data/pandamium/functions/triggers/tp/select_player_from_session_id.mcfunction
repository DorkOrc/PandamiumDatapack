...
execute if score <target_exists> variable matches 0 store success score <returned> variable run tellraw @s [{"text":"[TP]","color":"dark_red"},[{"text":" Could not find a player with Session ID ","color":"red"},{"score":{"name":"<target_session_id>","objective":"variable"}},"!"]]

scoreboard players operation @s tp = <target_id> variable
