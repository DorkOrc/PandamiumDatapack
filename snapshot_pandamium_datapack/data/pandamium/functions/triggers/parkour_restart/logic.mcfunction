...
# confirmation
execute if score @s parkour_restart matches 1.. if score @s parkour.checkpoint matches 201..299 run return run tellraw @s [{"text":"[Parkour]","color":"dark_red"},[{"text":" Are you sure you want to ","color":"red"},{"text":"reset all progress","underlined":true}," for this run of the ",{"storage":"pandamium:dictionary","nbt":"parkour.parkour_3.name","interpret":true,"color":"aqua"}," parkour course? "],{"text":"[✔]","color":"dark_green","clickEvent":{"action":"run_command","value":"/trigger parkour_restart set -101"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"dark_green"},{"text":"reset all progress","bold":true}," for this run of the ",{"storage":"pandamium:dictionary","nbt":"parkour.parkour_3.name","interpret":true,"bold":true}," parkour course"]}}]

# do restart
execute unless score @s parkour_restart matches 1.. unless score @s parkour_restart matches -101 run return run tellraw @s [{"text":"[Parkour]","color":"dark_red"},{"text":" That is not a valid option!","color":"red"}]
