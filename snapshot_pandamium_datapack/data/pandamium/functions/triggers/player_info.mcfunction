...
execute if score <player_info.returned> variable matches 0 store success score <player_info.returned> variable if score @s player_info matches -1 run tellraw @s [{"text":"[Player Info]","color":"dark_red"},{"text":" Could not find a non-staff player nearby!","color":"red"}]
...
execute if score <player_info.returned> variable matches 0 if score @s player_info matches 1.. store success score <player_info.returned> variable run function pandamium:impl/triggers/player_info/print_menu

# Click Events
execute if score <player_info.returned> variable matches 0 if score @s player_info matches ..-2 run scoreboard players set <player_info.target_exists> variable 0
execute if score <player_info.returned> variable matches 0 if score @s player_info matches 1.. run scoreboard players operation <player_info.target_id> variable = @s selected_player
execute if score <player_info.returned> variable matches 0 if score @s player_info matches ..-2 as @a if score @s id = <player_info.target_id> variable store success score <player_info.target_exists> variable run tag @s add player_info.target
execute if score <player_info.returned> variable matches 0 if score @s player_info matches ..-2 if score <player_info.target_exists> variable matches 0 store success score <player_info.returned> variable run tellraw @s [{"text":"[Player Info]","color":"dark_red"},{"text":" You have not selected a player or the player you have selected is offline!","color":"red"}]
execute if score <player_info.returned> variable matches 0 if score @s player_info matches ..-2 store success score <player_info.returned> variable run function pandamium:impl/triggers/player_info/click_events
