...
execute if score @s parkour.restart matches 1.. if score @s parkour.checkpoint matches 101..199 run tellraw @s [{"text":"[Parkour] ","color":"dark_red"},[{"text":"Are you sure you want to ","color":"red"},{"text":"reset all progress","underlined":true}," for Monstrosity Parkour? "],{"text":"[✔]","bold":true,"color":"dark_green","clickEvent":{"action":"run_command","value":"trigger parkour.restart set -101"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"dark_green"},{"text":"reset all progress","bold":true}," for Monstrosity Parkour"]}}]

execute if score @s parkour.restart matches -101 if score @s parkour.checkpoint matches 0..99 run function pandamium:misc/parkour/parkour_1/restart
...
execute if score @s parkour.restart matches -101 if score @s parkour.checkpoint matches 100..199 run function pandamium:misc/parkour/parkour_2/progress/restart
