...
execute if score @s gift matches 1 run function pandamium:impl/triggers/gift/print_menu

execute store success score <returned> variable unless score @s gift matches 2..
execute if score <returned> variable matches 0 store success score <returned> variable if score @s gift_cooldown matches 1.. run function pandamium:impl/triggers/gift/print_cooldown
...
# Run
execute if score <returned> variable matches 0 as @a[tag=target,limit=1] at @s run function pandamium:impl/triggers/gift/receive_vote_credit
execute if score <returned> variable matches 0 run scoreboard players remove @s vote_credits 1
execute if score <returned> variable matches 0 run tellraw @s [{"text":"","color":"green"},{"text":"[Gift]","color":"blue"}," Gifted ",{"selector":"@a[tag=target,limit=1]"}," a ",{"text":"vote credit","color":"aqua"},"! You now have ",{"score":{"name":"@s","objective":"vote_credits"},"bold":true,"color":"aqua"}," vote credit",{"text":"(s)","color":"gray"},"! ",[{"text":"(-","color":"red"},{"text":"1","bold":true},")"]]
