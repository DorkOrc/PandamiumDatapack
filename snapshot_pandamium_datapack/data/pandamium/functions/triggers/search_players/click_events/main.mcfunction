...
# validate
execute if data storage pandamium.db:click_events selected.entry{expired:1b} run return run tellraw @s [{"text":"[Search Players]","color":"dark_red"},{"text":" That option has expired!","color":"red"}]
...
execute if score <owner_id> variable = @s id if data storage pandamium.db:click_events selected.entry{trigger:"search_players"} run scoreboard players set <valid_click_event> variable 1
