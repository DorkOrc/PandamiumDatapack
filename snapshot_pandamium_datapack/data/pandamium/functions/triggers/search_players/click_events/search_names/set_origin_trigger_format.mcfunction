...
execute if score <origin_trigger_is_mail> variable matches 1 store result score <joined_since_mail_added> variable run function pandamium:triggers/mail/check_if_user_can_receive_mail with storage pandamium.db.players:io selected.entry
execute if score <origin_trigger_is_mail> variable matches 1 if score @s staff_perms matches 2.. run scoreboard players set <joined_since_mail_added> variable 1
execute if score <origin_trigger_is_mail> variable matches 1 if score <joined_since_mail_added> variable matches 0 run data modify block 3 0 0 front_text.messages[0] set value '["",{"text":"[✉+]","color":"gray","hoverEvent":{"action":"show_text","contents":[{"text":"","color":"red"},{"storage":"pandamium:temp","nbt":"display_name","interpret":true}," has not joined since mailing was enabled! Once they join the game at least once, they will be elligible for receiving mail."]}}," ",{"storage":"pandamium:temp","nbt":"display_name","interpret":true}]'
