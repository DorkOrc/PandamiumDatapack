...
execute if score @s options matches -8..-1 run scoreboard players set <valid_option> variable 1
...
execute if score @s options matches -1002 run tellraw @s [{"text":"[Options]","color":"dark_red"},[{"text":" Are you sure you want to ","color":"red"},[{"text":"enable ","underlined":true},{"text":"Hide Coordinates","bold":true}],"? You should only use this option if you are recording or streaming and do not want others to see your position. "],{"text":"[✔]","bold":true,"color":"dark_green","hoverEvent":{"action":"show_text","value":[{"text":"Click to cycle options for\n","color":"dark_green"},{"text":"Hide Coordinates","bold":true}]},"clickEvent":{"action":"run_command","value":"/trigger options set -8"}}]
