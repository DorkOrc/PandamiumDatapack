...
$execute unless data storage pandamium.db.players:data username_indexes."$(main_name)" run tellraw @s {"text":"Error: \"$(main_name)\" does not exist in the players database.","color":"red"}
$execute unless data storage pandamium.db.players:data username_indexes."$(main_name)" run return 0
$execute unless data storage pandamium.db.players:data username_indexes."$(alt_name)" run tellraw @s {"text":"Error: \"$(alt_name)\" does not exist in the players database.","color":"red"}
$execute unless data storage pandamium.db.players:data username_indexes."$(alt_name)" run return 0
