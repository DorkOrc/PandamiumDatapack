...
execute as @a[scores={detect.death=1..,death_particles=1..}] unless score @s hidden matches 1.. at @s if entity @e[type=player,distance=..64] run function pandamium:misc/particles/death_event
execute as @a[scores={detect.death=1..,disable_keep_inventory=1},gamemode=survival] in pandamium:staff_world if blocks 0 0 0 0 0 0 0 0 0 all run function pandamium:misc/drop_inventory_items
