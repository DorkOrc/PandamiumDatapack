...
execute store success score <spectator_mode> variable if score @s tp matches ..-1
...
execute if score <player_exists> variable matches 0 run tellraw @s [{"text":"[TP]","color":"dark_red"},{"text":" Could not find that player!","color":"red"}]

execute if score <player_exists> variable matches 1 if score <spectator_mode> variable matches 1 run gamemode spectator
