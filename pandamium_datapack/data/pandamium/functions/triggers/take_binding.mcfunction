...
execute if score @s temp_3 matches 1 if score @s temp_2 matches 1 if score @s temp_1 matches 1 at @a if score @s take_binding = @p id as @p run tellraw @s [{"text":"You took ","color":"green"},{"selector":"@p"},"'s binded items."]

execute if score @s temp_3 matches 1 if score @p temp_2 matches 1 if score @s temp_1 matches 0 run tellraw @s [{"text": "[Take Binding] ", "color":"dark_red"},{"selector":"@p"},{"text":" has no binded items to transfer.","color":"red"}]
