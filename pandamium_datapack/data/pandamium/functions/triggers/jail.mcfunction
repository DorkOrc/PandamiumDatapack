...
execute if score @s jail matches 2.. if score <player_exists> variable matches 1 if score <can_jail> variable matches 0 run tellraw @s [{"text":"[Jail]","color":"dark_red"},{"text":" This player is already jailed!","color":"red"}]
...
execute if score <player_exists> variable matches 1 if score <can_jail> variable matches 1 run scoreboard players operation @p[tag=selected_player] jailed = @s id
