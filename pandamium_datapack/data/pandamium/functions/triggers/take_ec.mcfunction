...
execute if score @s take_ec matches 2.. if score <player_exists> variable matches 1 if score <has_items> variable matches 1.. if score <empty_chest> variable matches 1 as @a if score @p[tag=running_trigger] take_ec = @s id in pandamium:staff_world run function pandamium:take/move_ec

#errors
execute if score @s take_ec matches 2.. if score <player_exists> variable matches 0 run tellraw @p[tag=running_trigger] [{"text":"[Info]","color":"dark_red"},{"text":" No player was found!","color":"red"}]
