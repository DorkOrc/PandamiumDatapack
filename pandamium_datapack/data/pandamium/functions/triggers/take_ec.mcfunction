...
execute as @a if score @p[tag=doing_trigger] take_ec = @s id if score <empty_chest> variable matches 1 if score <has_items> variable matches 0 run tellraw @p[tag=doing_trigger] [{"text": "[Info]", "color":"dark_red"}," ",{"selector":"@s","color":"red"},{"text":" has no items in their enderchest.","color":"red"}]
