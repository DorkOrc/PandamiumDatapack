...
execute at @a if score @s take_ec = @p id if score <has_items> variable matches 1.. if score <empty_chest> variable matches 1 as @p in minecraft:overworld run function pandamium:take/move_ec
execute at @a if score @s take_ec = @p id if score <has_items> variable matches 1.. if score <empty_chest> variable matches 1 run tellraw @s [{"text":"","color":"yellow"},{"text":"[Info]","color":"gold"}," Took ",[{"selector":"@p"},"'s"]," ",{"text":"enderchest","bold":true}," items!"]
