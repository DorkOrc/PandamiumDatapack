...
execute as @a if score @p[tag=running_trigger] take_inv = @s id run scoreboard players set <player_exists> variable 1

execute as @a if score @p[tag=running_trigger] take_inv = @s id if data entity @s Inventory[0] run scoreboard players set <has_items> variable 1

execute in pandamium:staff_world unless block 7 64 -5 chest run setblock 7 64 -5 minecraft:chest[facing=west,type=right]
execute in pandamium:staff_world unless block 7 64 -4 chest run setblock 7 64 -4 minecraft:chest[facing=west,type=left]
...
execute if score <player_exists> variable matches 1 if score <empty_chest> variable matches 1 if score <has_items> variable matches 1 as @a if score @p[tag=running_trigger] take_inv = @s id in pandamium:staff_world run function pandamium:take/move_inv
...
execute if score <player_exists> variable matches 1 if score <empty_chest> variable matches 1 if score <has_items> variable matches 0 run tellraw @p[tag=running_trigger] [{"text": "[Info]", "color":"dark_red"}," ",{"selector":"@s","color":"red"},{"text":" has no items in their inventory!","color":"red"}]
