...
execute at @a if score @s take_inv = @p id if score <has_items> variable matches 1.. if score <empty_chest> variable matches 2 as @p run function pandamium:take/move_inv
execute at @a if score @s take_inv = @p id if score <has_items> variable matches 1.. if score <empty_chest> variable matches 2 run tellraw @s [{"text":"","color":"yellow"},{"text":"[Info]","color":"gold"}," Took ",[{"selector":"@p"},"'s"]," ",{"text":"inventory","color":"aqua"}," items!"]
