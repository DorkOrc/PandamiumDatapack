...
execute if score <has_run> variable matches 0 unless score <displayed_error> variable matches 1 store success score <displayed_error> variable if score @s in_spawn matches 1 run tellraw @s [{"text":"[Frame Visibility]","color":"dark_red"},{"text":" You cannot use that trigger at spawn!","color":"red"}]
