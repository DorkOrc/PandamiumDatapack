...
execute if score @s vote_shop matches ..-1 if score <can_buy> variable matches 1 if score @s vote_credits matches 0 run tellraw @s [{"text":"","color":"green","clickEvent":{"action":"suggest_command","value":"/trigger vote"},"hoverEvent":{"action":"show_text","value":{"text":"Click to Vote for Pandamium!","color":"aqua"}}},{"text":"[Vote Shop]","color":"dark_green"}," ","Earn more vote credits by ",{"text":"voting","color":"aqua"}," up to ",{"text":"4 times a day","color":"aqua"},"! See the voting links with ",{"text":"/trigger vote","color":"yellow"},"!"]
