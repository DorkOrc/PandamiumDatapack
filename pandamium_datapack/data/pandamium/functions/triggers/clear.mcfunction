...
execute unless score @s jailed matches 1.. if score @s clear matches 1.. run tellraw @s [{"text":"","color":"red"},{"text":"[Clear]","color":"dark_red"}," Are you sure you want to ",{"text":"permanently delete all items","underlined":true}," from your inventory? ",{"text":"[✔]","bold":true,"color":"dark_green","clickEvent":{"action":"run_command","value":"/trigger clear set -100"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"dark_green"},{"text":"permanently delete all items","bold":true}," from your inventory"]}}]
