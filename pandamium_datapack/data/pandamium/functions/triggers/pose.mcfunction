...
execute if score <can_run> variable matches 1 if score @s pose matches -38..-37 if score <toggle_state> variable matches 1 run item replace entity @s weapon.mainhand with air
...
#Lock Armour Stand
execute if score <can_run> variable matches 1 if score @s pose matches -128 run tellraw @s [{"text":"[Pose]","color":"dark_red"},{"text":" Are you sure you want to lock the state of this armour stand? ","color":"red"},{"text":"[✔]","color":"dark_green","bold":true,"hoverEvent":{"action":"show_text","value":{"text":"Click to Lock the Pose of the Nearest Armour Stand","color":"dark_green"}},"clickEvent":{"action":"run_command","value":"/trigger pose set -129"}}]
