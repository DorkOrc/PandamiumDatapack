...
execute if score @s player_info matches 2.. if score <player_exists> temp matches 0 run tellraw @s [{"text":"[Player Info]","color":"dark_red"},{"text":" Could not find that player!","color":"red"}]

# Menu
execute if score <player_exists> temp matches 1 as @p[tag=selected_player] in pandamium:staff_world run function pandamium:misc/get_username
execute if score <player_exists> temp matches 1 run function pandamium:misc/player_info/menu
...
execute if score @s player_info matches ..-2 store success score <player_exists> temp if entity @p[tag=selected_player]
execute if score @s player_info matches ..-2 if score <player_exists> temp matches 1 run function pandamium:misc/player_info/click_events
execute if score @s player_info matches ..-2 if score <player_exists> temp matches 0 run tellraw @s [{"text":"[Spawnpoint]","color":"dark_red"},{"text":" You have not selected a player or the player you have selected is offline!","color":"red"}]
