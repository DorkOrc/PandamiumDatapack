...
scoreboard players set <selected_player> variable 0
execute if score @s staff_perms matches 2.. if score @s jail matches ..-2 run scoreboard players operation <selected_player> variable -= @s jail
execute if score @s staff_perms matches 2.. if score @s jail matches ..-2 as @a if score @s id = <selected_player> variable run tag @s add selected_player
...
execute if score <can_run> variable matches 1 if score <can_tp> variable matches 1 run tellraw @s [{"text":"","color":"yellow"},{"text":"[Jail]","color":"gold"}," Teleported to ",[{"selector":"@p[tag=selected_player]"},"'s"]," last ",{"text":"Pre-Jail Location","color":"gold","bold":true}," in ",{"text":"spectator mode","color":"gold","bold":true},"!"]
execute if score <can_run> variable matches 1 if score <can_tp> variable matches 0 run tellraw @s [{"text":"[Jail]","color":"dark_red"}," ",{"selector":"@p[tag=selected_player]","color":"red"},{"text":" has no pre-jail location stored!","color":"red"}]
