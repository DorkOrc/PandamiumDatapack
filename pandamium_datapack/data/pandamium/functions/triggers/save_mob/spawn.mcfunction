...
execute unless data storage pandamium:temp NBT.SelectedItem.tag.pandamium.stored_mob run tellraw @s [{"text":"[Save Mob]","color":"dark_red"},[{"text":" You must be holding a ","color":"red"},{"text":"Stored Mob Item","color":"gold"}," to run that trigger!"]]
