...
execute as @a if score @s id = @p[tag=running_trigger] tpa run scoreboard players set <player_exists> variable 1
...
execute if score @p[tag=running_trigger] jailed matches 1.. run tellraw @p[tag=running_trigger] [{"text":"[TPA]","color":"dark_red"},{"text":" You cannot use that trigger in jail!","color":"red"}]
execute unless score @p[tag=running_trigger] jailed matches 1.. if score <has_sent_request> variable matches 1 as @a if score @s tpa_request = @p[tag=running_trigger] id run tellraw @p[tag=running_trigger] [{"text":"","color":"red"},{"text":"[TPA]","color":"dark_red"}," You have already sent a TPA request to ",{"selector":"@s","color":"red"},"! ",{"text":"[X]","bold":true,"color":"gray","clickEvent":{"action":"run_command","value":"/trigger tpa set -3"},"hoverEvent":{"action":"show_text","value":[{"text":"Click to ","color":"gray"},{"text":"Cancel","bold":true}," outgoing TPA Request"]}}]
