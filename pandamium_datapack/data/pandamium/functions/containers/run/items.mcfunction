...
execute unless data storage pandamium:containers {container:'inventory'} unless data storage pandamium:containers {container:'enderchest'} run tellraw @s [{"text":"└Items: ","color":"aqua"},{"text":"[See Contents] ","color":"gray","hoverEvent":{"action":"show_text","value":["x",{"score":{"name":"<total_items>","objective":"variable"}}," Filled Slots"]}}]
