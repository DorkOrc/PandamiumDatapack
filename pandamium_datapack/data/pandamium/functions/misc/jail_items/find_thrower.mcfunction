...
execute if score <uuid_matches> variable matches 1 in pandamium:staff_world run data merge block 7 64 -6 {Text2:'[{"text":"Dropped by ","color":"gray","italic":false},{"selector":"@p[tag=thrower]","color":"gray"}]'}
execute if score <uuid_matches> variable matches 1 run tellraw @a[scores={staff_perms=1..}] [{"text":"[Info]","color":"dark_gray"}," ",{"selector":"@p[tag=thrower]"},{"text":" threw an item in jail!","color":"gray"}]
