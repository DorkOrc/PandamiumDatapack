...
execute unless data storage pandamium:temp NBT.data.RequiredCheckpoint if score @s parkour_checkpoint matches 0.. if data storage pandamium:temp NBT.data.Destination run function pandamium:misc/teleport/to_scores/parkour_main
execute if data storage pandamium:temp NBT.data.RequiredCheckpoint if score @s parkour_checkpoint >= <required_checkpoint> variable if data storage pandamium:temp NBT.data.Destination run function pandamium:misc/teleport/to_scores/parkour_main
